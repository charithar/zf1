...
abstract class Zend_Http_Client_CommonHttpTests extends \PHPUnit\Framework\TestCase
{
    ...
    public function testResetParameters()
    {
        ...
        $this->assertStringContainsStringIgnoringCase(serialize($params) . "\n" . serialize($params),
            $res->getBody(), "returned body does not contain all GET and POST parameters (it should!)");

        $this->client->resetParameters();
        $res = $this->client->request('POST');

        $this->assertStringNotContainsStringIgnoringCase(serialize($params), $res->getBody(),
            "returned body contains GET or POST parameters (it shouldn't!)");
        $this->assertStringContainsStringIgnoringCase($headers["X-Foo"], $this->client->getHeader("X-Foo"), "Header not preserved by reset");
    ...
    public function testParameterUnset()
    {
        ...
        $this->assertStringNotContainsStringIgnoringCase('cheese', $res->getBody(), 'The "cheese" GET parameter was expected to be unset');
        $this->assertStringNotContainsStringIgnoringCase('alice', $res->getBody(), 'The "to" POST parameter was expected to be unset');
    }
    ...
    public function testHeadersSingle()
    {
        ...
        foreach ($headers as $key => $val)
            $this->assertStringContainsStringIgnoringCase(strtolower("$key: $val"), $body);

        $this->assertStringContainsStringIgnoringCase(strtolower($acceptHeader), $body);
    }
    ...
    public function testHeadersArray()
    {
        ...
        foreach ($headers as $key => $val) {
            if (is_string($key)) {
                ...
                $this->assertStringContainsStringIgnoringCase(strtolower($val), $body);
            }
        }
     }
    ...
    public function testRedirectStrict()
    {
        ...
        $this->assertStringContainsStringIgnoringCase('swallow', $res->getBody());
        $this->assertStringContainsStringIgnoringCase('Camelot', $res->getBody());
    }
    ...
    public function testCancelAuth()
    {
        ...
        $this->assertEquals(401, $res->getStatus(), 'Expected HTTP 401 response was not recieved');
        $this->assertStringNotContainsStringIgnoringCase('alice', $res->getBody(), "Body contains the user name, but it shouldn't");
        $this->assertStringNotContainsStringIgnoringCase('secret', $res->getBody(), "Body contains the password, but it shouldn't");
    }
    ...
    public function testPutRequestsHonorSpecifiedContentType()
    {
        ...
        $this->assertStringContainsStringIgnoringCase('text/html; charset=ISO-8859-1', $request, $request);
        $this->assertStringContainsStringIgnoringCase('REQUEST_METHOD: PUT', $response->getBody(), $response->getBody());
    }
    ...
    public function testMultiplePuts()
    {
        ...
        $response = $this->client->request();
        $request= $this->client->getLastRequest();
        $this->assertStringNotContainsStringIgnoringCase('Content-Type: text/plain', $request);
