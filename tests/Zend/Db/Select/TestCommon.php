...
abstract class Zend_Db_Select_TestCommon extends Zend_Db_TestSetup
{
    ...
    public function testSelectColumnsReset()
    {
        ...
        $stmt = $this->_db->query($select);
        $result = $stmt->fetchAll();
        $this->assertStringContainsStringIgnoringCase('product_name', array_keys($result[0]));
        $this->assertStringNotContainsStringIgnoringCase('product_id', array_keys($result[0]));
    ...
    public function testJoinUsingActuallyGeneratesAnInnerJoinOnForCompatibilityReasons()
    {
        ...
        $this->assertStringContainsStringIgnoringCase("JOIN {$table_B} ON {$table_B}.{$colname} = {$table_A}.{$colname}", $s->assemble());
    }
